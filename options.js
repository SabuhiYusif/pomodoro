...
chrome.storage.local.get({'pomodoro': 25}, function(result) {
	pomodoro_selector = document.getElementById("pomodoro_selector");

	pomodoro_selector.value  = result.pomodoro;
	
})
chrome.storage.local.get({'short_br': 5}, function(result) {
...
chrome.storage.local.get({'long_br': 15}, function(result) {
...
btn.onclick = function(element) {
	...
	chrome.storage.local.set({'pomodoro': pomodoro_selector}, function() {
	})
	chrome.storage.local.set({'short_br': short_break_time_selector}, function() {
	})
	chrome.storage.local.set({'long_br': long_break_time_selector}, function() {
	})
	chrome.storage.local.set({'pomodoros': pomodoros}, function() {
	})
};
